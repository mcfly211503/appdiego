...
document.addEventListener("DOMContentLoaded", function() {
    // El código de tus botones y eventos va aquí
        ...
        categorias.forEach((categoria) => {
            ...
            filas.forEach((fila, index) => {
                ...
                if (cuentaCarreras > 0) {
                    ...
                    corredores.push({ nombre, numero, tiempoPromedio });
                } else {
                    corredores.push({ nombre, numero, tiempoPromedio: "No tiene tiempos válidos" });
                }
            });

            // Ordenar los resultados por tiempo
            corredores.sort((a, b) => {
                if (a.tiempoPromedio === "No tiene tiempos válidos") return 1;
                if (b.tiempoPromedio === "No tiene tiempos válidos") return -1;
                return parseFloat(a.tiempoPromedio) - parseFloat(b.tiempoPromedio);
            });

            corredores.forEach((corredor) => {
                resultados += `<p>Corredor ${corredor.numero} - ${corredor.nombre}: Tiempo Promedio: ${corredor.tiempoPromedio} segundos</p>`;
            });
        });
